...
func (m *MemberDomain) Get(ctx context.Context, uuid uuid.UUID) (*aggregate.Member, error) {
	...
	err = dbcore.GetDB(ctx).Debug().Model(&relations).Association("Role").Find(&roles).Error()
	err = dbcore.GetDB(ctx).Debug().Model(&relations).Association("User").Find(&user).Error()
