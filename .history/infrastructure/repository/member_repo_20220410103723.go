...
type Profile struct {
	ID      uuid.UUID `gorm:"primarykey"`
	Name    string    `gorm:"index;default:0;"`   //
	Usert   Usert     `gorm:"foreignkey:UsertID"` // 使用 RoleID 作为外键
...
func (m *MemberDomain) Get(ctx context.Context, uuid uuid.UUID) (*aggregate.Member, error) {
	...
	dbcore.GetDB(ctx).Debug().Where("id=?", "b4fcb682-f023-419f-9c5b-f67124da7257").Find(&profile)

	db := dbcore.GetDB(ctx).Debug().Model(&profile).Association("UsertID").Find(&user)
